...
const FindId = () => {
    ...
    const findId = () => {
        ...
        axios.post('http://43.200.222.222:8080/member/find/id',{
            name :koName,
            phone_num: phone.replace(/\-/g,'')
          }).then((res) => {
              if(res.data.result === "OK"){ //성공
                setFindIdClick(true)
                //email.replace(new RegExp('.(?=.{0,'+(email.split('@')[0].length - 3)+'}@)', 'g'), '*')
                if(res.data.data.split('@')[0].length > 5){
                    setEmail(res.data.data.replace(new RegExp('.(?=.{0,3}@)', 'g'), '*'))
                }else{
                    setEmail(res.data.data.replace(new RegExp('.(?=.{0,1}@)', 'g'), '*'))
                }    
    ...
    return (
        ...
        <Row className="align-items-center mb-3">
            ...
            {/* 휴대폰번호 인증요청 버튼 */}
                <Col sm = {3}>
                    <div className='d-grid join_btns'>
                        <Button name='phoneBtn' variant={'warning'} size="lg" disabled={!koName || !phone || certification ? true : false}
            ...
            <Row>
                <Col sm = {12}>
                    <div id="buttonGroup">
                        <Button variant="outline-primary" onClick={() => findId()} disabled={!certification || email ? true : false}>
                            아이디 찾기
                        </Button>{' '}
                        <Link to = {"/auth/login"}>
