...
import { changeField,initializeForm, partner } from "../../modules/authJoin";
...
const PartnerJoin = () => {
    ...
    const [managerChecked, setManagerChecked] = useState(0); // 사업자인증번호확인 - 미확인 - 0 ,실패 - 1  확인 - 2
    ...
    const [requestBusinessNum, setRequestBusinessNum] = useState(''); //전달하는 사업자등록번호 
    ...
    const handleBusinessNumChange = (e) => {
        ...
        if (regex.test(e.target.value)) {
            ...
            if(businessNumCheck.test(e.target.value)){
                ...
                console.log('business_num : ', e.target.value);
            }else{
    ...
    const handleOpenDateChange=(e) =>{
        ...
        if(dateCheck.test(e.target.value)){
            ...
            console.log('opening_day : ', e.target.value);
        }else{
            setIsOpenDate(false);
        }
    }
    //사업자관리번호 조회
    const handdleBusinessNumCheckClick=(e)=>{//수정필요
         axios.post('http://43.200.222.222:8080/',{
            
        }).then((res) =>{
            if(!res.data.data){ //조회완료
                setManagerChecked(2);
                setRequestBusinessNum((businessNum.replace(/-/g, '')));
                dispatch(
                    changeField({
                        form : 'partner',
                        key : 'name',
                ...
                console.log('사업자조회 및 dispatch ', Name, businessNum.replace(/-/g, ''), openDate);
            }else{// 조회실패
                setManagerChecked(1);
    ...
    return(
        ...
        <Row className="align-items-center mb-3">
            ...
            {/* 이메일 중복확인 버튼 */}
            <Col sm = {3}>
                <div className={emailChecked === 2 ? "d-grid mx-auto": "d-grid join_btns mx-auto"}>
        ...
        <Row className="align-items-center mb-3">
                <Col sm = {12}>
                    <div className="d-grid">
                        <Button variant={isName && isBusinessNum && isOpenDate ?'info' : 'outline-info'} onClick={handdleBusinessNumCheckClick} size="md" disabled={(isName && isBusinessNum && isOpenDate) && managerChecked < 2 ? false : true}>
                            {managerChecked < 2 ? '사업자번호 확인요청' :'사업자번호 확인완료' }
                        </Button>
                    </div>
                </Col>
        </Row>
        ...
        <Row className="align-items-center mb-3">
               <Col sm = {12}>
                    ...
                    <JoinCheck value="1" getDataCheckbox1={highFunction1} getDataCheckbox2={highFunction2} />
                </Col>
        </Row>
        <Row className="justify-content-md-center mb-3">
            ...
            <form onSubmit={onSubmit}>
                <div className="d-grid">
                        ...
                        <Button type="submit" variant="primary" size="lg" onSubmit={onSubmit} disabled={(isCheckbox1 && isCheckbox2) ? false : true}>회원가입</Button>
                </div>
            </form>    
            </Col>
            <Col md="auto" />
...
export default PartnerJoin;//isEmail && emailChecked && isPwd && isPwdCheck && isName && isBusinessNum && isOpenDate && managerChecked  && isPhoneNum && iscertifyNum && certifyChecked &&